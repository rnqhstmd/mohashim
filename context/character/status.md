...
| FR-25 | ItemOverlay 캐릭터 레이어 시스템 — Z-index back/body/head/face. Inventory.equipped 반영 (메인 + 상점 미리보기). 미장착 슬롯 미렌더, previewItem이 동일 슬롯 equipped 대체 | ✅ | [PR #30](https://github.com/rnqhstmd/mohashim/pull/30). Phase 25. `popup/ItemOverlay.tsx` 신규 + FocusStartButton/PomodoroCard/ShopTab.PreviewArea 통합. MainScreen이 `inventory-updated` 구독으로 즉시 반영. 단위 테스트 9건(슬롯 매핑/미장착 미렌더/animated 토글/DOM 순서) |
