...
### 📤 잔디 자랑하기
한 달 잔디 그래프를 1080×1080 PNG으로 합성해 클립보드에 복사합니다.  
가장 집중 잘 한 날 / 할일 가장 많이 한 날 등 베스트 통계도 함께 들어갑니다.
...
### 집중도 점수 (0~100)
매초 두 축의 점수를 더해 총점을 산출합니다.

| 축 | 만점 | 산출 방식 |
|----|------|-----------|
| **활동 점수 (work)** | 80 | 키보드/마우스 입력이 끊긴 시간(idle)이 길어질수록 점수 감소. 짧은 자리비움(180초 이내)은 그대로 80점 유지 |
| **환경 점수 (noise)** | 20 | 마이크로 측정한 dB EMA가 65dB 이하면 만점, 80dB 초과 시 0점 |

5분 가까이 자리를 비우면 work=0, 시끄러운 환경에선 noise=0이 됩니다. 두 점수의 합으로 캐릭터 표정과 멘트가 5단계로 변합니다.

### 포모도로 사이클
`집중(25분)` → `휴식(5분)` → `세션 완료` 순서로 자동 전환됩니다. 시간은 설정에서 조절할 수 있어요.

- 집중 종료 시: `🍅 집중 종료!` OS 알림.
- 세션 완료 시: `🎉 세션 완료!` OS 알림 + 잔디에 기록.
- 시작 후 도중 그만두면 점수는 기록되지 않고 잔디에 빈 칸으로 남습니다.

### 메뉴바 / 트레이
앱은 항상 메뉴바(macOS) / 작업표시줄 트레이(Windows)에 상주합니다. 아이콘을 클릭하면 팝업이 아이콘 바로 아래/위에 정렬되어 나타나고, 다시 클릭하면 닫힙니다. 아이콘 자체도 5단계 캐릭터 표정으로 바뀌어 한눈에 상태를 알 수 있어요.

### 데이터 보관
모든 데이터는 로컬에만 저장됩니다 — 외부 서버 전송 없음. 마이크는 dB 수치만 1초 단위로 산출하며 음성 자체는 어디에도 저장되지 않습니다.

자세한 알고리즘과 아키텍처는 [개발 문서 (DEVELOPMENT.md)](DEVELOPMENT.md)에서 확인하세요.
